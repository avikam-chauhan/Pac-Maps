        ...
        for user in self.users {
            if user.UUID != UIDevice.current.identifierForVendor!.uuidString {
                if let index = self.ghostAnnotation.firstIndex(where: {(annotation) -> Bool in annotation.UUID == user.UUID }) {
                    ...
                    var annotation = CustomAnnotation(annotationType: .ghost_red, location: CLLocation(latitude: user.location!.latitude, longitude: user.location!.longitude), UUID: user.UUID)
                ...
                DispatchQueue.main.async {
                    if let mke = error as? MKError {
                        ...
                        case Int(MKError.Code.loadingThrottled.rawValue):
                            let alert = UIAlertController(title: "Please Try Again Later", message: "You have used too many routing requests in a short amount of time. Please wait for \(mke.errorUserInfo["timeUntilReset"]) seconds.", preferredStyle: UIAlertController.Style.alert)
